...
full.data <- fread("~/Desktop/Hi-C/full.data.10.Nov17", header = TRUE, data.table=FALSE, stringsAsFactors = FALSE, showProgress = FALSE)
data.4 <- fread("~/Desktop/Hi-C/data.4.10.Nov17", header=TRUE, data.table=FALSE, stringsAsFactors = FALSE, showProgress = FALSE)
...
#These are slightly better, but many still show some small degree of deflation. After looking at all these QQ plots our concerns about p-value inflation are fairly assuaged, and it's worth noting that we perhaps shouldn't have even been worried in the first place. We pulled out significant Hi-C hits from each species, so it makes sense that this set of hits might already be enriched for strong species differences. We conclude the signal seen in the QQ plot is not an artifact of making incorrect assumptions in linear modeling.
...
asym.stats <- data.frame(chr=unique(data.filtered$Hchr), binom.p=rep(NA, 24))
rownames(asym.stats) <- asym.stats$chr
for(chromo in unique(data.filtered$Hchr)){
  mydat <- filter(data.filtered, Hchr==chromo, sp_pval<=0.05) #Iterate through chromosomes
...
fwrite(full.data, "~/Desktop/Hi-C/full.data.10.Nov17", quote = TRUE, sep = "\t", row.names = FALSE, col.names = TRUE, na="NA", showProgress = FALSE)
fwrite(data.4, "~/Desktop/Hi-C/data.4.10.Nov17", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE, na="NA", showProgress = FALSE)

...
```
###Promoter vs non-promoter contacts distance analysis
```{r}
...
diffs <- H2s-H1s
proms.dist <- data.frame(distance=diffs, promoter=full.annotations$true.promoter)
ggplot(data=proms.dist) + geom_violin(aes(x=promoter, group=promoter, color=promoter, y=distance), draw_quantiles = c(0.25, 0.5, 0.75)) + ggtitle("Distance Between Mates of a Pair | Promoter-Contact Status") + xlab("Does Contact Involve a Promoter?") + ylab("Distance b/t Mates") + guides(color=guide_legend(title="Promoter-Contact?")) + annotate("text", x=1.5, y=3000000, label="< 2.2 e -16")
t.test(filter(proms.dist, promoter=="no")$distance, filter(proms.dist, promoter=="yes")$distance)
#Dope, very big difference, clear that promoter-involving contacts are a little more physically constrained.
```
...
ggplot(data=myrich) + geom_histogram(aes(x=chromstate, group=chromstate, fill=chromstate), stat="count") + coord_cartesian(ylim=c(0, 80000)) + ggtitle("Basal chromHMM State Assignments Distribution") + xlab("chromHMM State Assignment") + ylab("# Bins")
ggplot(data=weighter) + geom_histogram(aes(x=chromstate, group=chromstate, fill=chromstate), stat="count") + coord_cartesian(ylim=c(0, 80000)) + scale_x_discrete(drop=FALSE) + ggtitle("Weighted chromHMM State Assignments Distribution") + xlab("chromHMM State Assignment") + ylab("# Bins")

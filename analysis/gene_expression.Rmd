...
A_AB <- fread("~/Desktop/Hi-C/AB_compartments/A-21792.PC1.txt", header = TRUE, data.table=FALSE)
B_AB <- fread("~/Desktop/Hi-C/AB_compartments/B-28126.PC1.txt", header = TRUE, data.table=FALSE)
E_AB <- fread("~/Desktop/Hi-C/AB_compartments/E-28815.PC1.txt", header = TRUE, data.table=FALSE)
F_AB <- fread("~/Desktop/Hi-C/AB_compartments/F-28834.PC1.txt", header = TRUE, data.table=FALSE)
C_AB <- fread("~/Desktop/Hi-C/AB_compartments/C-3649.PC1.txt", header = TRUE, data.table=FALSE)
D_AB <- fread("~/Desktop/Hi-C/AB_compartments/D-40300.PC1.txt", header = TRUE, data.table=FALSE)
G_AB <- fread("~/Desktop/Hi-C/AB_compartments/G-3624.PC1.txt", header = TRUE, data.table=FALSE)
H_AB <- fread("~/Desktop/Hi-C/AB_compartments/H-3651.PC1.txt", header = TRUE, data.table=FALSE)
...
hgene.hic <- left_join(hgene.hic, human_lengths, by="genes")
cgene.hic <- left_join(cgene.hic, chimp_lengths, by="genes")
...
hgene.hic$HID <- paste(hgene.hic$Hchr, ifelse(hgene.hic$Hstrand=="+", ifelse(hgene.hic$Hstart%%50000>=(50000-hgene.hic$length), as.integer(50000*ceiling(hgene.hic$Hstart/50000)), as.integer(50000*floor(hgene.hic$Hstart/50000))), ifelse(hgene.hic$Hstart%%50000<=hgene.hic$length, as.integer((50000*floor(hgene.hic$Hstart/50000)-50000)), as.integer(50000*floor(hgene.hic$Hstart/50000)))), sep="-")
cgene.hic$CID <- paste(cgene.hic$Cchr, ifelse(cgene.hic$Cstrand=="+", ifelse(cgene.hic$Cstart%%50000>=(50000-cgene.hic$length), as.integer(50000*ceiling(cgene.hic$Cstart/50000)), as.integer(50000*floor(cgene.hic$Cstart/50000))), ifelse(cgene.hic$Cstart%%50000<=cgene.hic$length, as.integer((50000*floor(cgene.hic$Cstart/50000)-50000)), as.integer(50000*floor(cgene.hic$Cstart/50000)))), sep="-")
...
hgenes.compartments <- select(hgene.hic, Hchr, Hstart, genes, Hstrand, HID)
cgenes.compartments <- select(cgene.hic, Cchr, Cstart, genes, Cstrand, CID)
...
```{r}
A_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/A-21792_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
B_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/B-28126_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
C_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/C-3649_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
D_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/D-40300_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
E_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/E-28815_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
F_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/F-28834_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
G_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/G-3624_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)
H_TAD <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/H-3651_TOPDOM_ALL.domain", header=TRUE, data.table=FALSE)

#Can ask a variety of different questions. Start simple and just get a table of TAD boundary crossings for individual contacts/genes within species.
...
#Rather than worry about how to create a standardized set of boundaries or other elements here (lots of inter-individual variance), start by using ALL boundary calls as real boundaries and see results. Note that this was very annoying because the formatting was off for some reason, have to use awk to print these files out and then tail -n +2 each of them (awk -v OFS="\t" '{print $1, $2, $3, $4, $5, $6, $7, $8}' TOPDOM/human.boundaries.bed > TOPDOM/hbounds.bed). The tail -n +2 has to write to a completely new file for some reason. The ultimate command run is akin to (bedtools pairtobed -type notispan -a filt.bins.humans.bedPE -b TOPDOM/hbounds.final.bed > TOPDOM/hg38.TD.bounds.notispan.bed)
human.bounds <- rbind(A_bounds, B_bounds, E_bounds, F_bounds)
human.bounds <- human.bounds[!duplicated(human.bounds),]
human.bounds$strand <- "."
human.bounds <- human.bounds[,c(1, 3, 5, 2, 4, 8, 6, 7)]
human.bounds <- format(human.bounds, scientific=FALSE)
colnames(human.bounds) <- c("chromosome", "start", "end", "frombin", "tobin", "strand", "tag", "size")
fwrite(human.bounds, "~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/human.boundaries.bed", sep="\t", na="NA", col.names=TRUE, quote=FALSE)
...
chimp.bounds <- rbind(C_bounds, D_bounds, G_bounds, H_bounds)
chimp.bounds <- chimp.bounds[!duplicated(chimp.bounds),]
chimp.bounds$strand <- "."
chimp.bounds <- chimp.bounds[,c(1, 3, 5, 2, 4, 8, 6, 7)]
chimp.bounds <- format(chimp.bounds, scientific=FALSE)
colnames(chimp.bounds) <- c("chromosome", "start", "end", "frombin", "tobin", "strand", "tag", "size")
fwrite(chimp.bounds, "~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/chimp.boundaries.bed", sep="\t", na="NA", col.names=TRUE, quote=FALSE)

#Load in the pairs that are NOT interrupted by a TAD boundary!
TADcross.humans <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/hg38.TD.bounds.notispan.bed")
TADcross.chimps <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/pt5.TD.bounds.notispan.bed")
...
table(full.annotations$DE, full.annotations$diff.TAD.interrupt, dnn=c("DE?", "TAD Boundary Cross Diff. b/t Species?"))
chisq.test(table(full.annotations$DE, full.annotations$diff.TAD.interrupt))
#About 18.1% of DE-involved contacts cross TAD boundaries differently between the species; only 14.6% of the non-DE contacts do this.

table(filter(full.annotations, true.promoter=="yes")$DE, filter(full.annotations, true.promoter=="yes")$diff.TAD.interrupt, dnn=c("DE?", "TAD Boundary Cross Diff. b/t Species?"))
chisq.test(table(filter(full.annotations, true.promoter=="yes")$DE, filter(full.annotations, true.promoter=="yes")$diff.TAD.interrupt, dnn=c("DE?", "TAD Boundary Cross Diff. b/t Species?")))

table(full.annotations$DC, full.annotations$diff.TAD.interrupt, dnn=c("DC?", "TAD Boundary Cross Diff. b/t Species?"))
chisq.test(table(full.annotations$DC, full.annotations$diff.TAD.interrupt))

table(filter(full.annotations, true.promoter=="yes")$DC, filter(full.annotations, true.promoter=="yes")$diff.TAD.interrupt, dnn=c("DC?", "TAD Boundary Cross Diff. b/t Species?"))
chisq.test(table(filter(full.annotations, true.promoter=="yes")$DC, filter(full.annotations, true.promoter=="yes")$diff.TAD.interrupt, dnn=c("DC?", "TAD Boundary Cross Diff. b/t Species?")))
```

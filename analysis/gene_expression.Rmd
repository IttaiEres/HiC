...
enrichment.plotter <- function(df, HiC_col, DE_col, xlab, i=c(0.01, 0.05, 0.1), k=seq(0.01, 1, 0.01)){
  enrich.table <- data.frame(DEFDR = c(rep(i[1], 100), rep(i[2], 100), rep(i[3], 100)), DHICFDR=rep(k, 3), prop.obs=NA, prop.exp=NA, chisq.p=NA, Dneither=NA, DE=NA, DHiC=NA, Dboth=NA)
  for(de.FDR in i){
    for(hic.FDR in k){
  ...
  des.enriched <- ggplot(data=enrich.table, aes(x=DHICFDR, y=prop.obs*100, group=as.factor(DEFDR), color=as.factor(DEFDR))) +geom_line()+ geom_line(aes(y=prop.exp*100), linetype="dashed", size=0.5) + ggtitle("Enrichment of DHiC in DE Genes") + xlab(xlab) + ylab("Proportion of DE genes that are DHiC") + guides(color=guide_legend(title="FDR for DE Genes"))
...
minFDRexp <- fread("~/Desktop/HiC_covs/results/permout_h_minFDR", header = TRUE, stringsAsFactors = FALSE)
minFDRe <- minFDRexp/sum(minFDRexp[1,])
minFDRo <- lmcorrect(hmin[,c("A-21792", "B-28126", "C-3649", "D-40300", "E-28815", "F-28834", "G-3624", "H-3651")], hmin[,c("A_21792_HIC", "B_28126_HIC", "C_3649_HIC", "D_40300_HIC", "E_28815_HIC", "F_28834_HIC", "G_3624_HIC", "H_3651_HIC")], meta.data)
minFDRo <- data.frame(DEneither=sum(h_minFDR_pvals$before>0.05&h_minFDR_pvals$after>0.05), DEbefore=sum(h_minFDR_pvals$before<=0.05&h_minFDR_pvals$after>0.05), DEafter=sum(h_minFDR_pvals$before>0.05&h_minFDR_pvals$after<=0.05), DEboth=sum(h_minFDR_pvals$before<=0.05&h_minFDR_pvals$after<=0.05))
minFDRo <- minFDRo/sum(minFDRo[1,])

ggplot(data=minFDRe, aes(x="", y=DEbefore*100)) + geom_boxplot(aes(color="Expected"), show.legend=FALSE) + geom_point(aes(y=minFDRo$DEbefore*100, color="Observed"), size=3) + ggtitle("Percent of genes with reduced evidence for DE after Hi-C correction") + ylab("% genes DE before, but not after, Hi-C correction") + xlab("1000 Permutations of Hi-C data") + scale_color_manual(values=c("blue", "red"), guide=FALSE) +guides(color=guide_legend("Data", override.aes = list(shape=c(16, 16))))#+ guides(color=guide_legend(title="Data", override.aes = list(shape=5))) #Finally have it a way where it just shows points in legend for both; better than showing boxplots for both.

ggplot(data=minFDRe, aes(x="", y=DEafter*100)) + geom_boxplot(aes(color="Expected"), show.legend=FALSE) + geom_point(aes(y=minFDRo$DEafter*100, color="Observed"), size=3) + ggtitle("Percent of genes with increased evidence for DE after Hi-C correction") + ylab("% genes DE after, but not before, Hi-C correction") + xlab("1000 Permutations of Hi-C data") + scale_color_manual(values=c("blue", "red"), guide=FALSE) +guides(color=guide_legend("Data", override.aes = list(shape=c(16, 16))))

ggplot(data=minFDRe, aes(x="", y=DEneither*100)) + geom_boxplot(aes(color="Expected"), show.legend=FALSE) + geom_point(aes(y=minFDRo$DEneither*100, color="Observed"), size=3) + ggtitle("Percent of genes with no evidence for DE regardless") + ylab("% genes not DE in either") + xlab("1000 Permutations of Hi-C data") + scale_color_manual(values=c("blue", "red"), guide=FALSE) +guides(color=guide_legend("Data", override.aes = list(shape=c(16, 16))))

ggplot(data=minFDRe, aes(x="", y=DEboth*100)) + geom_boxplot(aes(color="Expected"), show.legend=FALSE) + geom_point(aes(y=minFDRo$DEboth*100, color="Observed"), size=3) + ggtitle("Percent of genes with evidence for DE regardless") + ylab("% genes DE in both") + xlab("1000 Permutations of Hi-C data") + scale_color_manual(values=c("blue", "red"), guide=FALSE) +guides(color=guide_legend("Data", override.aes = list(shape=c(16, 16))))

...
lmFit(exprs, design=design) %>% eBayes(.) -> fit_full


#Filter to just DE genes here.
filter <- topTable(fit_full, coef=2, adjust="BH", number=Inf, sort.by="none")
good.indices <- which(filter$adj.P.Val <= 0.05)

#Now run analyses
exprs <- exprs[good.indices,]
hic <- hic[good.indices,]
...
plot(x=make_plot[,1], y=make_plot[,2], ylab="Standard error of the difference", xlab="Difference in effect size from 2 lms", main="Difference of effect size (DE genes)", pch=16, cex=0.6, col=make_plot[,3])
...
plot(x=make_plot[,1], y=make_plot[,2], ylab="Standard error of the difference", xlab="Difference in effect size from 2 lms", main="Difference of effect size (non-DE genes)", pch=16, cex=0.6, col=make_plot[,3])

...
TADcross.humans.2 <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/hu.TD.bounds.notispan.2.bed")
TADcross.humans.3 <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/hu.TD.bounds.notispan.3.bed")
TADcross.humans.4 <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/hu.TD.bounds.notispan.4.bed")

TADcross.chimps <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/pt5.TD.bounds.notispan.bed")
TADcross.chimps.2 <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/ch.TD.bounds.notispan.2.bed")
TADcross.chimps.3 <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/ch.TD.bounds.notispan.3.bed")
TADcross.chimps.4 <- fread("~/Desktop/Hi-C/pgltoolswork/new_updated/TOPDOM/ch.TD.bounds.notispan.4.bed")
...
exprs <- as.data.frame(filter(hmin, adj.P.Val<=0.05) %>% select(., "A-21792", "B-28126", "C-3649", "D-40300", "E-28815", "F-28834", "G-3624", "H-3651", genes)) #Should be done on just DE genes, filter with adj.P.Val<=0.05
...
#lmFit(myweight, design=design) -> fit_b2 #for some reason, running it with this makes it awful. VERY different sigmas...perhaps b/c this was already a voom object?
#lmFit(residweight, design=design) -> fit_a

lmFit(exprs, design=design) -> fit_b
lmFit(resid_hic, design=design) -> fit_a
...
design[c(3:4, 7:8),2] <- -1 #Because of the transposing for getting A below, need to have chimps be -1

#Get variances of beta_before's species effect:
se_beta_b <- fit_b$sigma*sqrt((solve(t(design)%*%design))[2,2])

head(cbind(se_beta_b,fit_b$sigma*sqrt(fit_b$cov.coefficients[2,2])))
...
beta_diff <- beta_b-beta_a #Everything written out by LEB's example was checked up to here, so that's where something must be different.

#Now run some of that ASH!
fit_ash <- ash(as.vector(beta_diff), cov_diff_sqrt, mode=0)

summary(fit_ash$result$svalue) #For some reason these are slightly inflated still compared to Lauren's analysis.
summary(fit_ash$result$svalue<0.05) #As a result I have fewer here true than hers (528 vs 339)
...
plot(x=make_plot[,1], y=make_plot[,2], ylab="Standard error of the difference", xlab="Difference in effect size from 2 lms", main="Difference of effect size (DE genes)", pch=16, cex=0.6, col=make_plot[,3], xlim=c(-2.5, 2.5), ylim=c(0, 1.3))
...
plot(x=make_plot[,1], y=make_plot[,2], ylab="Standard error of the difference", xlab="Difference in effect size from 2 lms", main="Difference of effect size (non-DE genes)", pch=16, cex=0.6, col=make_plot[,3], xlim=c(-2, 2))
